...
cap = cv2.VideoCapture('C:/Users/PC/PycharmProjects/ANPR/Dataset/video3.mp4')
#extra addition
...
while True:
    ...
    if text:
        ...
        def register_vehicle():
            with open("numberplates_data.csv", "a") as f:
                writer = csv.writer(f)
                writer.writerow([cleaned_text])
                print("number registerd")

            # messagebox.showinfo("Number Registered", "Number registered successfully")
            # window.destroy()
        ...
        if cleaned_text in number_plates:
            ...
            label.pack()
            btn = tk.Button(text="Register Vehicle", command=register_vehicle)
            btn.pack()
